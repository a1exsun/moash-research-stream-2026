...
Further, I think there might be a "cross-over effect": to build a continual learning architecture, we might have to sacrifice some of the natural data compression and generalization power of the Transformer. This is because global attention and backpropagation give the model great initial power but naturally limit its continual learning performance. If this new architecture can beat a standard Transformer of the same size in its target area after some time, then it is valuable.
