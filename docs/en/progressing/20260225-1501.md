...
- **Refactored Proposal 04 (CDCL Curvature Decomposition)**: Searching for "empty rooms" within massive Dense parameters is equivalent to finding the Null Space or flat directions of the Fisher matrix. CDCL serves as the mathematical toolkit to precisely map out these "implicitly idle sub-spaces" within the Dense space, restricting parameter movements exclusively within them.
- **Refactored Proposal 05 (Capacity Ceiling)**: This acts as the ultimate physical constraint. It signals that even perfectly exploited implicit sparsity has a theoretical upper bound of $C \cdot B$ in a single-regime system. However, this highlights that before we hit that distant ceiling, discovering and allocating existing implicit sparsity remains an extremely valuable gold mine.
