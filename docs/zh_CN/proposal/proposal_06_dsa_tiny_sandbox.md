# Proposal 06: 面向持续学习的 Tiny-DSA 沙盒预训练验证

## 研究动机

如果"后天修改既有模型的注意力结构"（如 Proposal 04 讨论的方向）会面临预训练分布与新机制之间的不兼容问题，从而缺乏控制变量的说服力；那么，从零开始构建一个基于 DeepSeek 原生动态稀疏注意力（DSA: Dynamic Sparse Attention）架构的极小沙盒模型，可能是一条更干净的机制验证路径。

由于核心目标是检验"分离的注意力通路是否有助于缓解遗忘"这一假说，可以脱离生产级的大参数模型，在极小尺度（~100M 参数）的语料集（如 TinyStories）上进行端到端的初步验证。
