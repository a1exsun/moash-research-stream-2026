...
- 哪种机制对缓解遗忘的贡献最大？
- 这两种机制（结构保护与数据回放）叠加时，是否具备显著的正向协同效应？
...
针对基于 nanoGPT (GPT-2 Small, 124M) 规模的实验算力耗费估算：
...
- **单轮耗时**：`7.44e17 / 1.5e14 = 4960 秒` ≈ **1.4 小时 (单张 A100)**。如果使用单张消费级显卡（如 RTX 4090）也通常在 2-3 小时内。
...
- **资金与时间成本**：
  ...
  - 使用单卡 A100，**仅需不到 1 天** 即可跑完所有 12 组实验。使用消费级单卡（RTX 4090）也仅需 1.5 天左右。
