# Crossover Hypothesis: 结构性稀疏拓扑作为持续学习基础条件的实证研究
...
LLM在静态训练范式下展现出了卓越的能力，但它们共享一个根本性的缺陷：无法从部署后的持续经验中学习。当一个LLM在新数据上进行微调时，它会迅速丧失先前学到的知识——这一现象被称为灾难性遗忘（catastrophic forgetting），自McCloskey和Cohen（1989）首次报告以来，三十多年过去了，问题依然没有被真正解决。
...
本研究的起点是一个简单的问题：**持续学习的本质问题是什么？**

一个模型在t=0时学到了函数f₀，在t=1时需要学到f₁，要求f₁既包含新知识又保留旧知识中有用的部分。在dense网络中，f₀编码在所有参数的联合配置中，修改任何参数都可能破坏f₀。这是灾难性遗忘的根因。

解决这个问题在逻辑上有几种可能的路径？

1. 保护旧参数（EWC、正交投影）→ 容量耗尽，不可扩展
2. 隔离新旧参数（LoRA、adapter、memory layer）→ 不整合，本质是外挂
3. 减小修改的影响半径（稀疏连接）→ 修改一个参数只影响局部计算路径
4. 让模型学会自我整合（replay、sleep consolidation）→ 需要额外机制

本质上，只有路径3直接作用于根因。路径1和2是回避问题，路径4是事后补救。理论上来说，假设网络密度为ρ，一个参数w_ij被修改后，受影响的计算路径数量正比于ρ^(L-1)，L是网络深度。当ρ=1（dense）时，影响是全局的。当ρ=0.01时，影响呈指数级缩小。这是一个数学事实，不依赖任何假设。
...
- **突触稳态假说：** Tononi和Cirelli提出，睡眠期间大脑进行突触下调，弱化不重要的连接、保护重要的连接——这是一种在稀疏拓扑上的竞争性选择过程。
- **神经发生：** 成人海马体持续产生新神经元，提供新的学习容量——这是一种在稀疏网络中添加新节点和连接的扩展机制。

这些观察引导我提出一个根本性的假设：**稀疏连接拓扑不是人脑持续学习的附属特征，而是使持续学习成为可能的必要条件。**
...
最初我的研究方向是设计一个仿脑的多机制协同架构，一个transformer架构的优化方案，通过引入类似人脑的运行机制来解决灾难性遗忘的问题。例如，清醒-睡眠循环、竞争性突触下调、拓扑重构、稀疏连接。

关键的反思时刻来自一个简单的检验：**如果解决灾难性遗忘需要同时使用3-4种机制，那很可能还没有找到问题的本质。** 真正的瓶颈应该是单一的。

这促使我追问：在所有这些机制中，哪一个是真正不可替代的？哪些只是在弥补某个根本缺陷的后果？
...
在排除多余机制的过程中，我也审视了另一条看似优雅的路径：正交子空间学习。
...
这引向了我的核心洞察：**也许问题不在于"如何在密集网络中找到安全的更新方向"，而在于"为什么我要在一个全局耦合的架构中寻找局部更新的空间"。**
...
这引导我重新定义研究目标。不是追问"如何在不遗忘的前提下学习新任务"，而是追问：**是否存在一种架构，以较差的初始性能为代价，换取持续学习的能力，使得经过足够多轮学习后，其累积优势超过初始劣势？**

我将此称为"交叉曲线假说"（Crossover Hypothesis）：
...
在深入文献后，我发现一个关键的概念混淆需要澄清：**稀疏更新（sparse updates）和稀疏拓扑（sparse topology）是两个根本不同的概念。**
...
在明确了研究方向后，我进行了系统的文献检索，结果令人惊讶：**那个最基础的消融实验从未被执行过。**
...
**第二类：结构化稀疏性 + 正则化。** Dual Lottery Ticket（2025）在Llama-3-8B上使用expander graph mask（10%稀疏度）结合EWC做持续学习，发现结构化的expander mask显著优于随机mask。但他们没有测试去掉EWC后单靠稀疏拓扑的效果——我永远无法从他们的结果中分离出稀疏性本身的贡献。
...
**应对：** 根据CHT的经验（1% MNIST成功，30% LLaMA-130M勉强匹配），语言建模可能需要20-30%密度。但在新范式下，我接受初始性能差距，因此可以探索更激进的密度。如果1-5%确实不可行，10-20%仍有研究价值——只要遗忘减少的程度在实践中有意义。

### 风险二：稀疏性不是遗忘的充分条件

**表现：** 稀疏网络的遗忘虽然减少，但没有实质性消除。

**应对：** 这本身就是有价值的负面结果。Cobweb/4V（2025）的结论——"no definitive evidence for the effect of sparsity"——正是因为他们没有在标准transformer上做这个实验。我在transformer上的负面结果，加上对原因的分析（如高阶干扰、有效秩不足），将为社区提供重要的实证证据。
...
灾难性遗忘的根本原因在于密集网络的全局参数耦合。本研究提出，结构性稀疏拓扑通过物理性地限制干扰传播范围，可能是实现持续学习的必要架构条件。通过一系列控制变量的消融实验，我将首次直接度量稀疏拓扑对遗忘的独立影响，验证交叉曲线假说，并为持续学习社区提供一个被长期忽视的架构级视角。

无论实验结果是正面还是负面，这项工作都将填补一个重要的实证空白——不是因为它提出了最巧妙的算法，而是因为它回答了一个最基本的问题：**在通往持续学习的道路上，我是否从一开始就选错了架构？**
