...
本研究的起点是一个简单的问题：**为什么密集神经网络在架构层面对持续学习存在系统性不利因素？**
...
这些观察引导我们提出一个根本性的假设：**稀疏连接拓扑不是人脑持续学习的附属特征，而是实现低干扰持续学习的重要结构性归纳偏置。**
...
**稀疏连接拓扑：** 这是物理层面的干扰隔离。当一个权重 $w_{ij}$ 被修改时，受影响的计算路径数量直接由网络密度 $\rho$ 决定。在“独立随机稀疏、无残差与全局注意力混合”的理想化条件下，深度为 $L$ 的网络中受影响路径比例可近似写为 $\rho^{L-1}$。在真实Transformer中，由于残差连接与注意力的全局信息混合，实际干扰传播会偏离该近似，因此需要通过实验直接估计有效干扰半径。
...
在明确了研究方向后，我们进行了系统的文献检索，得到的工作性判断是：**据我们当前检索范围，尚未发现在标准Transformer上完整执行该最小消融协议的工作。**
...
**核心空白：** 那个最简洁、最干净的实验——**同一个transformer架构，固定密度，不加任何额外机制（不冻结权重、不加EWC、不做参数隔离），直接在序列化数据上训练，对比dense vs. sparse的遗忘曲线**——据我们当前检索范围，尚未看到完整对照。大量现有论文在稀疏性之上叠加了其他机制，导致难以分离"稀疏拓扑本身对遗忘的影响"这一基本因果关系。
...
> 结构性稀疏连接拓扑是实现低遗忘长期持续学习的强约束条件（strong inductive bias）。在等预算且不依赖大规模历史回放的条件下，密集网络的长期保持能力预计仍受全局参数耦合限制。

**可证伪条件：** 如果在等参数/等FLOPs预算下，密集网络在长序列持续学习中持续达到与结构稀疏拓扑相当或更好的“保持率-学习速度”联合指标，则该假说不成立。
...
- 公平性约束：同时报告三组配对结果，避免容量混淆：
  - **等形状配对（same-shape）：** 相同层数与隐藏维，比较dense与不同密度sparse
  - **等非零参数配对（matched-nonzero）：** 构建参数量与稀疏网络非零参数相当的dense对照
  - **等训练FLOPs配对（matched-FLOPs）：** 控制训练计算预算一致
...
- 联合效率指标：在固定训练预算下的“保持率/学习速度”比值

**统计协议：**
- 每组实验固定至少3个随机种子
- 报告均值、标准差与95%置信区间
- 关键结论以预注册主指标（平均遗忘率与前向迁移）为准

**域数据选择：**
- 采用自然语言的域转移场景，如：代码 → 数学 → 医学 → 法律 → 对话
- 使用公开语料库构建各域数据集

### 4.1.1 实验一B：现实外推性检验（最小机制基线）

**目标：** 在保留“拓扑因果消融”的同时，检验结论在常见持续学习实践中的可迁移性。

**基线设置（与实验一A共享数据与预算）：**
- Dense + LoRA
- Dense + Replay（固定小缓存）
- Sparse-topology + LoRA

**评估重点：**
- 稀疏拓扑带来的增益是否在常见工程机制下依然存在
- 拓扑效应与算法机制效应是否可叠加
...
1. **据我们检索范围，首批系统实证"干扰半径"假说：** 通过控制变量实验，直接度量稀疏拓扑对参数干扰传播范围的影响，建立密度-遗忘率的定量关系。
...
**本研究的独特定位：** 据我们当前检索范围，在标准transformer架构上系统执行该类纯消融并分离结构性稀疏拓扑独立影响的工作仍然稀缺。本研究将以可复现对照协议补足这一基础实证空白。

---

## 九、总结

灾难性遗忘的重要来源之一在于密集网络的全局参数耦合。本研究提出，结构性稀疏拓扑通过物理性地限制干扰传播范围，可能是提升持续学习稳定性的关键架构条件。通过一系列控制变量的消融实验，我们将系统度量稀疏拓扑对遗忘的独立影响，验证交叉曲线假说，并为持续学习社区提供一个被长期忽视的架构级视角。
